...
use crate::crypto::constants::des_cons::{E, FP, IP, KEY_56, P, PC1, PC2, S_BOXES as S};
...
pub fn des(string_to_cipher: &str) -> String {
    ...
    for chunk in ciphered_chunks {
        let chunk_ciphered = apply_permutation(&chunk, &FP);
...
pub fn apply_round(chunk: &str, key: &str) -> String {
    ...
    let chunk_right_expanded = apply_permutation(&chunk_right, &E);

    // Now we need to XOR the expanded right half with the key
    let chunk_right_expanded_xor_key = xor(&chunk_right_expanded, &key);

    // Now we need to apply the S-boxes
    let chunk_right_expanded_xor_key_sbox = apply_sboxes(&chunk_right_expanded_xor_key);

    // Now we need to apply the permutation P
    let permuted = apply_permutation(&chunk_right_expanded_xor_key_sbox, &P);

    // Now we need to XOR the result with the left half
    let result = xor(&permuted, &chunk_left);
...
pub fn apply_sbox(chunk: &str, i: usize) -> String {
    ...
    let column = chunk.chars().skip(1).take(4).collect::<String>();
...
pub fn generate_k_keys(key_56: &str) -> Vec<String> {
    ...
    let key_56_pc1 = apply_permutation(key_56, &PC1);
    ...
    for i in 0..16 {
        ...
        let key_48 = apply_permutation(&format!("{}{}", key_l_shifted, key_r_shifted), &PC2);
...
pub fn split_key(key: &str) -> (String, String) {
    let key_left = key.chars().take(key.len() / 2).collect::<String>();
    let key_right = key.chars().skip(key.len() / 2).collect::<String>();
